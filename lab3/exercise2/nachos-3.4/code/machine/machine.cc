...
int  Machine::alloFrame(void){
	int pos=0;
	for( ; pos< NumPhysPages; pos++){
		if(bitmap[pos]==0){
			bitmap[pos]=1;
			printf("Allocating success! %d \n", pos);
			return pos;

		}
		DEBUG('a', "Frame  %d, have been used!\n", pos);
	}

	return -1;
}

void Machine::freeFrame(void){

	printf("FreeFrame current thread: %d \n", currentThread->getThreadID());
	for(int i=0;i< machine->pageTableSize;i++){
		int pos = pageTable[i].physicalPage;
		if(bitmap[pos]==1){
			bitmap[pos]=0;
			printf("Deallocating success! %d \n", pos);
		}
	}
}
