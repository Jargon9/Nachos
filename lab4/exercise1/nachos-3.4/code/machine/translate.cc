...
// from the virtual address
	...
	entry = &pageTable[vpn];
        ...
        for (entry = NULL, i = 0; i < TLBSize; i++){
    	    if (tlb[i].valid && (tlb[i].virtualPage == vpn)&&tlb[i].threadID==currentThread->getThreadID()) {
    	    	entry = &tlb[i];
    		    DEBUG('a', "virtual page %d Find in TLB %d!\n",vpn,i
    		    		);// FOUND!
    		    DEBUG('a', "TLB.valid: %d  VP: %d, ppn: %d\n", tlb[i].valid,
