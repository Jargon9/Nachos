...
#include "system.h"
#include "unistd.h"
extern  int ThreadID_Mark[128];
extern Thread *Threadpointer[128];
...
Thread::Thread(char* threadName, int threadpriority)
{
    ...
    this->userID =getuid(); //something wrong with it //Init userID
    for(int i=0;i<128;i++){
    	if(ThreadID_Mark[i]==0){
    		ThreadID_Mark[i]=1;   //Initialize  ThreadID
    		this->threadID=i;
    	    Threadpointer[this->threadID]=this;
    		break;
    	}
    	if(i==127){
    		printf("Error!!!  Reach the limitation.   \n");
    		this->threadID=-1;  //ID = -1 Error
    		this->userID=-1;
    		ASSERT(false);
    	}
    }
...
{
	Thread::Thread(threadName,0);  //default
}
...
Thread::~Thread()
{
    ...
    ThreadID_Mark[this->threadID]=0; //Deallocate the thread id
    Threadpointer[this->threadID]=NULL;
